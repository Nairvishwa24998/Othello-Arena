    ...
    def othello_run_bot_v_bot_matches(
        ...
        for game_number in range(1, rounds + 1):
            ...
            if outcome == 1:
                ...
                draws += 1
                result_str = "Draw"
            black_piece_count = game.get_player_piece_count(MOVE_B)
            white_piece_count = game.get_player_piece_count(MOVE_W)
            self.bot_contest_data.append(f"Round {game_number}: {result_str}")
            print(f"Black Pieces: {black_piece_count}")
            print(f"White Pieces: {white_piece_count}")
